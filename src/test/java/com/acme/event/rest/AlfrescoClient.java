 ...
 * <p>
 * This client provides methods for authenticating with Alfresco and creating
 * file and folder within the Alfresco repository. It uses OkHttp as the
 * HTTP client and Jackson for JSON parsing.
 */
@Service
public class AlfrescoClient {
	...
	/**
	 * Obtains an authentication ticket from Alfresco using the default admin
	 * credentials.
	 *
	...
	private String getAlfrescoAuthTicket() throws IOException {
		...
		Request request = new Request.Builder().url(alfrescoUrl + AUTH_PATH).post(body).build();
	...
	/**
	 * Creates an file of cm:content type in the Alfresco repository.
	 *
	...
	public String createFile(String name) throws IOException {
		String jsonBody = String.format("""
				...
				""", name);
		return createNode(jsonBody);
	}

	/**
	 * Creates a folder in the Alfresco repository.
	 *
	...
	/**
	 * Creates a node in the Alfresco repository using the provided JSON body.
	 *
	 * @param jsonBody JSON string representing the file properties and metadata
	 * @return the name of the created node in Alfresco
	 * @throws IOException if there is an error during the HTTP request
	 */
	private String createNode(String jsonBody) throws IOException {
		String authTicket = getAlfrescoAuthTicket();
		RequestBody body = RequestBody.create(jsonBody, JSON_MEDIA_TYPE);
		Request request = new Request.Builder().url(alfrescoUrl + FILE_CREATION_PATH)
				.header("Authorization", "Basic " + encodeCredentials(authTicket)).post(body).build();

		try (Response response = CLIENT.newCall(request).execute()) {
			if (!response.isSuccessful()) {
				throw new IOException("Failed to create node in Alfresco: " + response);
			}
			JsonNode jsonNode = OBJECT_MAPPER.readTree(response.body().string());
